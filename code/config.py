...
class Config:
    ...
    # Key Vault secret mapping for this agent (subset of platform map)
    ...
    def _load_keyvault_secrets(cls):
        ...
        for attr, ref in getattr(cls, "KEY_VAULT_SECRET_MAP", []):
            ...
            secret_map[secret_name].append((attr, json_key))

        secrets = {}
        for secret_name, refs in secret_map.items():
            try:
                ...
                has_json_key = any(jk is not None for _, jk in refs)
                if has_json_key:
                    try:
                        data = json.loads(raw)
                    except Exception:
...
# List of all config variables required by the agent and observability
CONFIG_VARIABLES = [
    # General
    "ENVIRONMENT",

    # Azure Key Vault Configuration (always from .env)
    "USE_KEY_VAULT",
    "KEY_VAULT_URI",
    "AZURE_USE_DEFAULT_CREDENTIAL",

    # Azure Service Principal (for Key Vault auth, only if AZURE_USE_DEFAULT_CREDENTIAL=False)
    "AZURE_TENANT_ID",
    "AZURE_CLIENT_ID",
    "AZURE_CLIENT_SECRET",
    ...
    # LLM Model List (for observability cost calculation)
    "LLM_MODELS",
...
def _initialize_config():
    ...
    # Azure AI Search variables: always from .env, never from Key Vault
    ...
    for var_name in CONFIG_VARIABLES:
        ...
        if var_name in ("CONTENT_SAFETY_ENABLED",):
        ...
        if value is None or value == "":
            if var_name != "OBS_AZURE_SQL_TRUST_SERVER_CERTIFICATE":
                ...
                value = "" if var_name not in ("LLM_TEMPERATURE", "LLM_MAX_TOKENS", "OBS_AZURE_SQL_PORT") else None
